...
if __name__ == '__main__':
    ...
    #####################################################################SSA################################################################################################
        for adv_train_epoch in range(0,10):
            ...
            ###################### Generate Adversarial Signals based on SSA
            ...
            print('Test accuracy:', score[1])
            print('Done')
            np.save("adv_SSA_overshoot0.01_{}.npy".format(adv_train_epoch), adv_SSA)
            print('Done')
            ################################## Adversarial Training Begins
            model.fit(adv_SSA, teY[:length_attack], batch_size=batch_size, epochs=1, validation_split=0.1, shuffle=True)
            model.save('17oldmodel_dnn_clean10_AT_{}.h5'.format(adv_train_epoch))
            model.save_weights('17oldweights_dnn_clean10_AT_{}.h5'.format(adv_train_epoch))
            score = model.evaluate(adv_SSA, teY[:length_attack], verbose=0)
            SCORE1.append(score)
