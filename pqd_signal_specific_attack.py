...
if __name__ == '__main__':
    ...
    with sess.as_default():
        ...
        adv_DF = np.load("adv_SSA.npy")
        length_attack = 25000

        pqd_predict_normal = np.argmax(model.predict(teX), axis=1)
        pqd_predict_attack_df = np.argmax(model.predict(adv_DF), axis=1)
        ...
        pqd_17_predict_attack = np.argmax(model.predict(adv_DF[pqd_17_index,]), axis=1) ### the new prediction after attack
        fig = plt.figure()
        for i in range(17):
            plt.subplot(3, 6, i+1)
            plt.plot(teX[pqd_17_index[i],], 'b',label = 'Original')
            plt.plot(adv_DF[pqd_17_index[i],], 'r',label = 'SSA')
            plt.legend()
