...
if __name__ == '__main__':
    ...
    #########################################################FGSM Adversarial Signals Generating####################################################
        ...
        # np.save("confusion_matrix_FGSM.npy", np.array(C2, dtype=float))
    ...
    ##########################################################Samples Generated by FGSMk############################################################
        ...
        # pqd_17_index = [35, 31, 30, 3, 5, 1, 35, 31, 30, 3, 5, 1,35, 31, 30, 3, 5, 1]  ###from clss 0-16（1-17）
        ...
        pqd_17_predict_normal = np.argmax(model.predict(teX[pqd_17_index,]),
                                          axis=1)  ### the original prediction before attack
        pqd_17_predict_attack = np.argmax(model.predict(adv_SSA[pqd_17_index,]),
                                          axis=1)
        ...
        pqd_17_predict_attack_uni = np.argmax(model.predict(adv_univer[pqd_17_index,]),
                                      axis=1)
        ...
        for i in range(17):
            if 0<=i<=4:
                ...
                plt.legend()
                plt.title("C-{},{}\n(C-{},{})".format(pqd_17_predict_attack[i] + 1, pqd_type[pqd_17_predict_attack[i]],
                ...
                plt.legend()
                plt.title("C-{},{}\n(C-{},{})".format(pqd_17_predict_attack_uni[i] + 1, pqd_type[pqd_17_predict_attack_uni[i]],
                                                      pqd_17_predict_normal[i] + 1, pqd_type[pqd_17_predict_normal[i]]),
                          fontsize=10)
        ...
        pqd_17_predict_normal = np.argmax(model.predict(teX[pqd_17_index,]),
                                          axis=1)  ### the original prediction before attack
        pqd_17_predict_attack = np.argmax(model.predict(adv_univer[pqd_17_index,]),
