...
if __name__ == '__main__':
    ...
    ##########################################################Samples Generated by FGSM/SSA/SAA############################################################
        pqd_17_index = [13, 14, 31, 67, 37, 1, 13, 14, 31, 67, 37, 1, 13, 14, 31, 67, 37, 1]

        pqd_17_predict_normal = np.argmax(model.predict(teX[pqd_17_index,]), axis=1)###original prediction before attack
        pqd_17_predict_attack_FGSM = np.argmax(model.predict(adv_FGSM[pqd_17_index,]), axis=1)###new prediction after attack

        adv_SSA = np.load("adv_SSA.npy")
        pqd_17_predict_normal = np.argmax(model.predict(teX[pqd_17_index,]), axis=1)
        pqd_17_predict_attack_SSA = np.argmax(model.predict(adv_SSA[pqd_17_index,]),axis=1)
        ...
        pqd_17_predict_attack_SAA = np.argmax(model.predict(adv_univer[pqd_17_index,]),axis=1)
        ...
        for i in range(17):
            if 0<=i<=4:
                ...
                plt.legend()
                plt.title("C-{},{}\n(C-{},{})".format(pqd_17_predict_attack_SSA[i] + 1, pqd_type[pqd_17_predict_attack_SSA[i]],
                                                      pqd_17_predict_normal[i] + 1, pqd_type[pqd_17_predict_normal[i]]),
                          fontsize=10)
            elif 12<=i<=16:
                plt.subplot(3, 6, i + 1)
                plt.plot(teX[pqd_17_index[i],], 'b--', label='Original')
                ...
                plt.legend()
                plt.title("C-{},{}\n(C-{},{})".format(pqd_17_predict_attack_SAA[i] + 1, pqd_type[pqd_17_predict_attack_SAA[i]],
                                                      pqd_17_predict_normal[i] + 1, pqd_type[pqd_17_predict_normal[i]]),
                          fontsize=10)
        ...
        plt.title("Universal Perturbations", fontsize=10)
        plt.legend()
